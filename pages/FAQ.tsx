...
import { ChevronDown, HelpCircle, Shield, Lock, Wifi, Code, Zap, Database, AlertTriangle, Fingerprint, Key, Heart } from 'lucide-react';
      ...
      <div className="space-y-4 animate-fade-in-up" style={{ animationDelay: '100ms' }}>
        {t.faq.items.map((faq, index) => {
          ...
          return (
            <div 
              key={index} 
              className={`bg-white dark:bg-slate-900 rounded-2xl transition-all duration-300 border ${
                isOpen 
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`p-2 rounded-xl transition-colors ${
                      isOpen ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500'
                  }`}>
                    {getIcon(faq.icon)}
                  </div>
