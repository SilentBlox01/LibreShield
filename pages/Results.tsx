...
import { AlertTriangle, ShieldCheck, ArrowRight, RefreshCw, CheckCircle, Smartphone, Globe, Wifi, Key } from 'lucide-react';

export const Results: React.FC = () => {
  const { userProfile, resetProfile, t } = useApp();
  ...
  }, [userProfile.completed, navigate]);
  ...
  const getCategoryIcon = (cat: string) => {
      if(cat === 'Móvil') return <Smartphone size={18} />;
      if(cat === 'Navegador') return <Globe size={18} />;
      if(cat === 'Wi-Fi y Redes') return <Wifi size={18} />;
      if(cat === 'Contraseñas') return <Key size={18} />;
